...
function advancedSearch(){let el=$("#advancedSearch");if(!el)return;el.innerHTML='<div class="dictbar"><input id="manualSearchNo" placeholder="Номер документа"><input id="manualSearchOwner" placeholder="Ответственный"><select id="manualSearchStatus"><option value="">Все статусы</option><option>Черновик</option><option>На согласовании</option><option>На подписании</option><option>Зарегистрирован</option><option>Закрыт</option></select><button data-manual-search>Применить</button><button data-save-search>Сохранить запрос</button></div><div id="savedSearchList"></div>';savedSearches()}
function savedSearches(){let el=$("#savedSearchList");if(!el)return;let s=get();s.savedSearches=s.savedSearches||[];el.innerHTML=s.savedSearches.map((x,i)=>'<button class="chip" data-load-search="'+i+'">'+esc(x.name)+'</button>').join("")}
function approvalQueues(){let el=$("#queueWorkspace");if(!el)return;let s=get(),d=s.docs||[],a=d.filter(x=>x.status==="На согласовании"),p=d.filter(x=>x.status==="На подписании");el.innerHTML='<div class="grid"><section><h3>На согласовании</h3>'+(a.map(x=>'<div class="row" data-doc="'+x.id+'"><div><strong>'+esc(x.number)+'</strong><span>'+esc(x.title)+'</span></div></div>').join("")||"<p>Очередь пуста</p>")+'</section><section><h3>На подписании</h3>'+(p.map(x=>'<div class="row" data-doc="'+x.id+'"><div><strong>'+esc(x.number)+'</strong><span>'+esc(x.title)+'</span></div></div>').join("")||"<p>Очередь пуста</p>")+"</section></div>"}
function barcode(){let n="EOSDO-"+Date.now().toString().slice(-10);alert("Штрихкод документа: "+n)}
document.addEventListener("DOMContentLoaded",()=>{installTypes();renderFields();calendar();assistants();advancedSearch();approvalQueues()});
...
document.addEventListener("click",e=>{
 if(e.target.closest("[data-manual-search]")){let q=[$("#manualSearchNo")?.value,$("#manualSearchOwner")?.value,$("#manualSearchStatus")?.value].filter(Boolean).join(" ");let main=$("#q");if(main){main.value=q;main.dispatchEvent(new Event("input",{bubbles:true}))}return}
 if(e.target.closest("[data-save-search]")){let s=get();s.savedSearches=s.savedSearches||[];let name=prompt("Название поискового запроса","Мой поиск");if(!name)return;s.savedSearches.push({name,no:$("#manualSearchNo")?.value||"",owner:$("#manualSearchOwner")?.value||"",status:$("#manualSearchStatus")?.value||""});put(s);savedSearches();return}
 let ls=e.target.closest("[data-load-search]");if(ls){let x=(get().savedSearches||[])[+ls.dataset.loadSearch];if(x){$("#manualSearchNo").value=x.no;$("#manualSearchOwner").value=x.owner;$("#manualSearchStatus").value=x.status}return}
